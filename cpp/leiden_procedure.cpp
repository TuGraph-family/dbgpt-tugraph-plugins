...
extern "C" bool Process(GraphDB& db, const std::string& request, std::string& response) {
    ...
    try {
        ...
        parse_from_json(leiden_val, "leiden_val", input);
    ...
    if (leiden_val != "") {
        olapondb.WriteToGraphDB<size_t>(label, leiden_val);
    }

    start_time = get_time();
    double output_cost = get_time() - start_time;

    std::set<size_t> community_set;
    for (auto v_label : label) {
        community_set.insert(v_label);
    }

    {
        ...
        output["community_id_list"] = community_set;
